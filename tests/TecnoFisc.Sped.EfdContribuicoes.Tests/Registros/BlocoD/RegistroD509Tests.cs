...
public sealed class RegistroD509Tests
{
    private static readonly IRegistroSpedCatalogo _catalogo =
        CatalogoBuilder.BuildFromAssembly(typeof(RegistroD509).Assembly);

    [Fact]
    public void Atributo_DeclaraD509_Nivel4_BlocoD()
    {
        var atributo = typeof(RegistroD509).GetCustomAttribute<RegistroSpedAttribute>();

        atributo.Should().NotBeNull();
        atributo!.Codigo.Should().Be("D509");
        atributo.Nivel.Should().Be(4);
        atributo.Bloco.Should().Be("D");
    }

    [Fact]
    public void Catalogo_ExpoeRegistroD509ComDoisCamposNaOrdem()
    {
        _catalogo.TentarObter("D509".AsSpan(), out var meta).Should().BeTrue();

        meta!.Codigo.Should().Be("D509");
    ...
    {
        _catalogo.TentarObter("D509".AsSpan(), out var meta);
        var registro = (RegistroD509)meta!.Fabrica();

        meta.Campos[0].Definidor(registro, "PROC-2021-001".AsSpan());
    ...
    {
        const string sped = "|D509|PROC-2021-001|1|\r\n";
    ...
    {
        const string sped = "|D509|ADM-RFB-2020-0099|3|\r\n";
