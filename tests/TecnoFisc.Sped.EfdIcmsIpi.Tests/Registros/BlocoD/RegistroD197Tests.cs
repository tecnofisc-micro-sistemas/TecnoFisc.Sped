...
/// <summary>
/// Sub-stage 8.129 — exercita a forma do <see cref="RegistroD197"/> contra o Guia Prático
/// EFD-ICMS/IPI V3.0.6 (p. 180): metadados de catálogo, mapeamento de campos e invariante
/// de round-trip parse → gerar → texto idêntico.
/// </summary>
public sealed class RegistroD197Tests
{
    private static readonly IRegistroSpedCatalogo _catalogo =
        CatalogoBuilder.BuildFromAssembly(typeof(RegistroD197).Assembly);
    ...
    [Fact]
    public void Atributo_DeclaraD197_Nivel4_BlocoD()
    {
        var atributo = typeof(RegistroD197).GetCustomAttribute<RegistroSpedAttribute>();

        atributo.Should().NotBeNull();
        atributo!.Codigo.Should().Be("D197");
        atributo.Nivel.Should().Be(4);
        atributo.Bloco.Should().Be("D");
    }

    [Fact]
    public void Catalogo_ExpoeRegistroD197Com7CamposNaOrdem()
    {
        _catalogo.TentarObter("D197".AsSpan(), out var meta).Should().BeTrue();

        meta!.Codigo.Should().Be("D197");
    ...
    {
        _catalogo.TentarObter("D197".AsSpan(), out var meta);
        var registro = (RegistroD197)meta!.Fabrica();
    ...
    {
        const string sped =
            "|D197|RS10000001|Ajuste ICMS-ST|PROD001|1000,00|12,00|120,00|0,00|\r\n";
