...
public sealed class RegistroC820Tests
{
    private static readonly IRegistroSpedCatalogo _catalogo =
        CatalogoBuilder.BuildFromAssembly(typeof(RegistroC820).Assembly);

    [Fact]
    public void Atributo_DeclaraC820_Nivel4_BlocoC()
    {
        var atributo = typeof(RegistroC820).GetCustomAttribute<RegistroSpedAttribute>();

        atributo.Should().NotBeNull();
        atributo!.Codigo.Should().Be("C820");
        atributo.Nivel.Should().Be(4);
        atributo.Bloco.Should().Be("C");
    }

    [Fact]
    public void Catalogo_ExpoeRegistroC820ComDozeCamposNaOrdem()
    {
        _catalogo.TentarObter("C820".AsSpan(), out var meta).Should().BeTrue();

        meta!.Codigo.Should().Be("C820");
        meta.Campos.Select(c => c.Nome).Should().Equal(
        [
            "Cfop", "VlItem", "CodItem", "CstPis",
            "QuantBcPis", "AliqPisQuant", "VlPis",
        ...
        meta.Campos[4].Obrigatorio.Should().BeFalse();  // QuantBcPis
        meta.Campos[5].Obrigatorio.Should().BeFalse();  // AliqPisQuant
    ...
    {
        _catalogo.TentarObter("C820".AsSpan(), out var meta);
        var registro = (RegistroC820)meta!.Fabrica();
        ...
        meta.Campos[3].Definidor(registro, "03".AsSpan());            // CstPis
        meta.Campos[4].Definidor(registro, "500,000".AsSpan());       // QuantBcPis
        meta.Campos[5].Definidor(registro, "0,0100".AsSpan());        // AliqPisQuant
        meta.Campos[6].Definidor(registro, "5,00".AsSpan());          // VlPis
        meta.Campos[7].Definidor(registro, "03".AsSpan());            // CstCofins
        meta.Campos[8].Definidor(registro, "500,000".AsSpan());       // QuantBcCofins
        meta.Campos[9].Definidor(registro, "0,0300".AsSpan());        // AliqCofinsQuant
        meta.Campos[10].Definidor(registro, "15,00".AsSpan());        // VlCofins
    ...
    {
        const string sped = "|C820|5102|1000,00|PROD001|49|500,000|0,0100|5,00|49|500,000|0,0300|15,00|3.1.01.001|\r\n";
    ...
    {
        const string sped = "|C820|5102|1000,00||49||||49|||||\r\n";
    ...
    {
        const string sped = "|C820|5102|800,00||99||||99|||||\r\n";
