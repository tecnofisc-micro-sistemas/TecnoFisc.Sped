...
public sealed class RegistroQ001Tests
{
    [Fact]
    public void Registro_ConformeManifesto()
    {
        AssertRegistroEcf.ConformsToManifest(new RegistroQ001(), "Q001", "1:1");
    }
    ...
    {
        var resultado = new ParserEcf().ParseLinha($"|Q001|{valor}|");

        resultado.Sucesso.Should().BeTrue();
        resultado.Valor.Should().BeOfType<RegistroQ001>()
    ...
    {
        var resultado = new ParserEcf().ParseLinha("|Q001|X|");

        resultado.Sucesso.Should().BeTrue();
        resultado.Valor.Should().BeOfType<RegistroQ001>()
            .Which.ErrosDeFormato.Should().ContainSingle(erro =>
                erro.Campo == nameof(RegistroQ001.IndDad) && erro.ValorBruto == "X");
    }
}
