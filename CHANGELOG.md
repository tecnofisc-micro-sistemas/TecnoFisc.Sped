...
- O reconhecimento dos sete registros descontinuados tem uma **contrapartida** para quem já lia arquivos históricos com `LenientLayout = true`. Antes, `|X300|000001|EXPORTACAO|1234,56|` virava um `RegistroNaoReconhecido` que **preservava a linha crua** em `LinhaCrua`, de onde dava para extrair as colunas à mão. Agora vira um `RegistroX300` **sem nenhuma propriedade e sem a linha crua** — o conteúdo das colunas é descartado na leitura. Em modo estrito a mudança é só ganho (antes abortava); com `LenientLayout` é uma troca: deixa de haver exceção e passa a haver o tipo certo, ao custo do texto original. Quem depende do texto deve, por ora, manter uma cópia da linha do lado de fora. O PR seguinte fecha a lacuna expondo as colunas não modeladas em `ColunasNaoModeladas`, sem quebrar quem lê hoje.
