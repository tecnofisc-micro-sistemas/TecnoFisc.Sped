...
/// <summary>
/// Sub-stage 8.180 — exercita a forma do <see cref="RegistroG001"/> contra o Guia Prático
/// EFD-ICMS/IPI V3.0.6 (p. 236): metadados de catálogo, mapeamento de campos e invariante
/// de round-trip parse → gerar → texto idêntico.
/// </summary>
public sealed class RegistroG001Tests
{
    private static readonly IRegistroSpedCatalogo _catalogo =
        CatalogoBuilder.BuildFromAssembly(typeof(RegistroG001).Assembly);
    ...
    [Fact]
    public void Atributo_DeclaraG001_Nivel1_BlocoG()
    {
        var atributo = typeof(RegistroG001).GetCustomAttribute<RegistroSpedAttribute>();

        atributo.Should().NotBeNull();
        atributo!.Codigo.Should().Be("G001");
        atributo.Nivel.Should().Be(1);
        atributo.Bloco.Should().Be("G");
    }

    [Fact]
    public void Catalogo_ExpoeRegistroG001ComUmCampoNaOrdem()
    {
        _catalogo.TentarObter("G001".AsSpan(), out var meta).Should().BeTrue();

        meta!.Codigo.Should().Be("G001");
    ...
    {
        _catalogo.TentarObter("G001".AsSpan(), out var meta);
        var registro = (RegistroG001)meta!.Fabrica();
