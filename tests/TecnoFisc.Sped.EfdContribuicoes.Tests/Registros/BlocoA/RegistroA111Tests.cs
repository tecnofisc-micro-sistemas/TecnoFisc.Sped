...
public sealed class RegistroA111Tests
{
    private static readonly IRegistroSpedCatalogo _catalogo =
        CatalogoBuilder.BuildFromAssembly(typeof(RegistroA111).Assembly);

    [Fact]
    public void Atributo_DeclaraCodigoA111_Nivel4_BlocoA()
    {
        var atributo = typeof(RegistroA111).GetCustomAttribute<RegistroSpedAttribute>();

        atributo.Should().NotBeNull();
        atributo!.Codigo.Should().Be("A111");
        atributo.Nivel.Should().Be(4);
        atributo.Bloco.Should().Be("A");
    }

    [Fact]
    public void Catalogo_ExpoeRegistroA111ComDoisCamposNaOrdem()
    {
        _catalogo.TentarObter("A111".AsSpan(), out var meta).Should().BeTrue();

        meta!.Codigo.Should().Be("A111");
        meta.Campos.Select(c => c.Nome).Should().Equal(["NumProc", "IndProc"]);
        meta.Campos.Select(c => c.Ordem).Should().Equal([2, 3]);
        meta.Campos[0].Tamanho.Should().Be(15);
    ...
    {
        _catalogo.TentarObter("A111".AsSpan(), out var meta);
        var registro = (RegistroA111)meta!.Fabrica();

        meta.Campos[0].Definidor(registro, "1234567890-1.01".AsSpan());
        meta.Campos[1].Definidor(registro, "1".AsSpan());

        registro.NumProc.Should().Be("1234567890-1.01");
    ...
    {
        const string sped = "|A111|1234567890-1.01|1|\r\n";
    ...
    {
        const string sped = "|A111|10166.720001/2020-31|3|\r\n";
