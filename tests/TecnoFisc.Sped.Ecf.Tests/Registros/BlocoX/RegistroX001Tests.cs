...
public sealed class RegistroX001Tests
{
    [Fact]
    public void Registro_ConformeManifesto()
    {
        AssertRegistroEcf.ConformsToManifest(new RegistroX001(), "X001", "1:1");
    }
    ...
    {
        var resultado = new ParserEcf().ParseLinha($"|X001|{valor}|");

        resultado.Sucesso.Should().BeTrue();
        resultado.Valor.Should().BeOfType<RegistroX001>()
    ...
    {
        var resultado = new ParserEcf().ParseLinha("|X001|X|");

        resultado.Sucesso.Should().BeTrue();
        resultado.Valor.Should().BeOfType<RegistroX001>()
            .Which.ErrosDeFormato.Should().ContainSingle(erro =>
                erro.Campo == nameof(RegistroX001.IndDad) && erro.ValorBruto == "X");
    }
}
