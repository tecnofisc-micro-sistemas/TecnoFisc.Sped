...
/// <summary>
/// Sub-stage 10.045 — exercita a forma do <see cref="RegistroJ001"/> contra o Manual de
/// Orientação do Leiaute 9 da ECD (p. 170): metadados de catálogo, mapeamento de campo e
...
public sealed class RegistroJ001Tests
{
    ...
    [Fact]
    public void Atributo_DeclaraJ001_Nivel1_BlocoJ()
    {
        var atributo = typeof(RegistroJ001).GetCustomAttribute<RegistroSpedAttribute>();

        atributo.Should().NotBeNull();
        atributo!.Codigo.Should().Be("J001");
        atributo.Nivel.Should().Be(1);
        atributo.Bloco.Should().Be("J");
    }

    [Fact]
    public void Catalogo_ExpoeRegistroJ001ComUmCampoNaOrdem()
    {
        _catalogo.TentarObter("J001".AsSpan(), out var meta).Should().BeTrue();

        meta!.Codigo.Should().Be("J001");
    ...
    {
        _catalogo.TentarObter("J001".AsSpan(), out var meta);
        var registro = (RegistroJ001)meta!.Fabrica();
