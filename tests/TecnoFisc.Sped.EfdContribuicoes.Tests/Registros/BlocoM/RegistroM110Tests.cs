...
public sealed class RegistroM110Tests
{
    private static readonly IRegistroSpedCatalogo _catalogo =
        CatalogoBuilder.BuildFromAssembly(typeof(RegistroM110).Assembly);

    [Fact]
    public void Atributo_DeclaraM110_Nivel3_BlocoM()
    {
        var atributo = typeof(RegistroM110).GetCustomAttribute<RegistroSpedAttribute>();

        atributo.Should().NotBeNull();
        atributo!.Codigo.Should().Be("M110");
        atributo.Nivel.Should().Be(3);
        atributo.Bloco.Should().Be("M");
    }

    [Fact]
    public void Catalogo_ExpoeRegistroM110Com6CamposNaOrdem()
    {
        _catalogo.TentarObter("M110".AsSpan(), out var meta).Should().BeTrue();

        meta!.Codigo.Should().Be("M110");
    ...
    {
        _catalogo.TentarObter("M110".AsSpan(), out var meta);
        var registro = (RegistroM110)meta!.Fabrica();
    ...
    {
        const string sped =
            "|M110|0|1500,00|02|PROC-2024-001|Devolução de compras|31012024|\r\n";
    ...
    {
        // Ajuste de acréscimo sem processo referenciado, descrição ou data de referência
        const string sped =
            "|M110|1|2750,00|01||||\r\n";
