...
public sealed class RegistroM115Tests
{
    private static readonly IRegistroSpedCatalogo _catalogo =
        CatalogoBuilder.BuildFromAssembly(typeof(RegistroM115).Assembly);

    [Fact]
    public void Atributo_DeclaraM115_Nivel4_BlocoM()
    {
        var atributo = typeof(RegistroM115).GetCustomAttribute<RegistroSpedAttribute>();

        atributo.Should().NotBeNull();
        atributo!.Codigo.Should().Be("M115");
        atributo.Nivel.Should().Be(4);
        atributo.Bloco.Should().Be("M");
    }

    [Fact]
    public void Catalogo_ExpoeRegistroM115Com8CamposNaOrdem()
    {
        _catalogo.TentarObter("M115".AsSpan(), out var meta).Should().BeTrue();

        meta!.Codigo.Should().Be("M115");
    ...
    {
        _catalogo.TentarObter("M115".AsSpan(), out var meta);
        var registro = (RegistroM115)meta!.Fabrica();
    ...
    {
        const string sped =
            "|M115|1500,00|50|30000,000|65,0000|15032024|Devolução de mercadoria|1.2.3.4.5|Informação adicional|\r\n";
