...
public sealed class RegistroM810Tests
{
    private static readonly IRegistroSpedCatalogo _catalogo =
        CatalogoBuilder.BuildFromAssembly(typeof(RegistroM810).Assembly);

    [Fact]
    public void Atributo_DeclaraM810_Nivel3_BlocoM()
    {
        var atributo = typeof(RegistroM810).GetCustomAttribute<RegistroSpedAttribute>();

        atributo.Should().NotBeNull();
        atributo!.Codigo.Should().Be("M810");
        atributo.Nivel.Should().Be(3);
        atributo.Bloco.Should().Be("M");
    }

    [Fact]
    public void Catalogo_ExpoeRegistroM810Com4CamposNaOrdem()
    {
        _catalogo.TentarObter("M810".AsSpan(), out var meta).Should().BeTrue();

        meta!.Codigo.Should().Be("M810");
    ...
    {
        _catalogo.TentarObter("M810".AsSpan(), out var meta);
        var registro = (RegistroM810)meta!.Fabrica();

        meta.Campos[0].Definidor(registro, "001".AsSpan());            // NatRec
        meta.Campos[1].Definidor(registro, "8000,00".AsSpan());        // VlRec
        meta.Campos[2].Definidor(registro, "4.1.2.002".AsSpan());      // CodCta
        meta.Campos[3].Definidor(registro, "Produto isento".AsSpan()); // DescCompl

        registro.NatRec.Should().Be("001");
        registro.VlRec.Should().Be(8000m);
        registro.CodCta.Should().Be("4.1.2.002");
    ...
    {
        const string sped =
            "|M810|001|8000,00|4.1.2.002|Produto isento|\r\n";
    ...
    {
        const string sped = "|M810|603|4000,00|||\r\n";
