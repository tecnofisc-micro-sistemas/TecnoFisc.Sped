...
/// <summary>
/// Sub-stage 8.187 — exercita a forma do <see cref="RegistroH001"/> contra o Guia Prático
/// EFD-ICMS/IPI V3.0.6 (p. 245): metadados de catálogo, mapeamento de campos e invariante
/// de round-trip parse → gerar → texto idêntico.
/// </summary>
public sealed class RegistroH001Tests
{
    private static readonly IRegistroSpedCatalogo _catalogo =
        CatalogoBuilder.BuildFromAssembly(typeof(RegistroH001).Assembly);
    ...
    [Fact]
    public void Atributo_DeclaraH001_Nivel1_BlocoH()
    {
        var atributo = typeof(RegistroH001).GetCustomAttribute<RegistroSpedAttribute>();

        atributo.Should().NotBeNull();
        atributo!.Codigo.Should().Be("H001");
        atributo.Nivel.Should().Be(1);
        atributo.Bloco.Should().Be("H");
    }

    [Fact]
    public void Catalogo_ExpoeRegistroH001ComUmCampoNaOrdem()
    {
        _catalogo.TentarObter("H001".AsSpan(), out var meta).Should().BeTrue();

        meta!.Codigo.Should().Be("H001");
    ...
    {
        _catalogo.TentarObter("H001".AsSpan(), out var meta);
        var registro = (RegistroH001)meta!.Fabrica();
