...
public sealed class RegistroD201Tests
{
    private static readonly IRegistroSpedCatalogo _catalogo =
        CatalogoBuilder.BuildFromAssembly(typeof(RegistroD201).Assembly);

    [Fact]
    public void Atributo_DeclaraD201_Nivel4_BlocoD()
    {
        var atributo = typeof(RegistroD201).GetCustomAttribute<RegistroSpedAttribute>();

        atributo.Should().NotBeNull();
        atributo!.Codigo.Should().Be("D201");
        atributo.Nivel.Should().Be(4);
        atributo.Bloco.Should().Be("D");
    }

    [Fact]
    public void Catalogo_ExpoeRegistroD201ComSeisCamposNaOrdem()
    {
        _catalogo.TentarObter("D201".AsSpan(), out var meta).Should().BeTrue();

        meta!.Codigo.Should().Be("D201");
        ...
        meta.Campos[4].Obrigatorio.Should().BeFalse();   // VlPis
    ...
    public async Task RoundTrip_ComTodosOsCampos_PreservaTextoCanonico()
    {
        ...
        const string sped = "|D201|49|50000,00|50000,00|1,6500|825,00|3.1.01.001|\r\n";
    ...
    {
        // CST 99 (Outras Operações) — sem base/alíquota/valor PIS.
        const string sped = "|D201|99|5000,00|||||\r\n";
