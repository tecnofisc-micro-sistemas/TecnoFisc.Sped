...
Release de estreia da **ECF (Escrituração Contábil Fiscal)** e primeira versão estável da família. O pacote `TecnoFisc.Sped.Ecf` entra **read-only**, com modelo tipado único do leiaute 12 e leitura dos leiautes 8 a 12; os guarda-chuvas `TecnoFisc.Sped.Txt` e `TecnoFisc.Sped` passam a agregá-lo. Junto vêm duas quebras no `TecnoFisc.Sped.Txt.Engine`/`TecnoFisc.Sped.Ecd` que afetam quem já consome a `0.9.0` — a unificação dos enums `IndicadorDebitoCredito`/`IndicadorTipoConta` e o quarto parâmetro obrigatório de `RegistroNaoReconhecido` — e é isso que promove a versão a `1.0.0` (ver **Quebrado** em cada seção para o passo de migração).
...
> A ECF **estreia** nesta `1.0.0`, então nenhuma versão anterior liberada em nuget.org é afetada. As duas quebras abaixo estão registradas porque contradizem API e comportamento já descritos nesta mesma seção durante o desenvolvimento — nenhum consumidor precisa migrar nada.
...
[1.0.0]: https://github.com/tecnofisc-micro-sistemas/TecnoFisc.Sped/releases/tag/v1.0.0
